...
// Permuted Choice 1 (PC-1) table for first 28 bits
...
// Permuted Choice 1 (PC-1) table for second 28 bits
...
void generate_keys(uint8_t key[DES_KEY_SIZE], uint8_t round_keys[16][6]) {
    uint8_t C[4] = {0}; // 28-bit left half
    uint8_t D[4] = {0}; // 28-bit right half
    uint64_t combined_key = 0;

    // Apply PC-1 to the key for C and D
    for (int i = 0; i < 28; i++) {
    ...
    // Apply left shifts and PC-2 for each round key
